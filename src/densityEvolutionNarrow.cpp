...
int main(int argc, char *argv[])
{
    ...
    {
        xs[i].x = xmin + (xmax-xmin)*float(i)/float(points);
        xs[i].nonLin = nonLin;
    ...
    {
        FileName = saveFolder + "logisticDensity-R" + std::to_string(nonLin) + "-P" + std::to_string(p) + "-W" + std::to_string(w) + ".csv";
    } else if (modelName == "tent")
    {
        FileName = saveFolder + "tentDensity-R" + std::to_string(nonLin) + "-P" + std::to_string(p) + "-W" + std::to_string(w) + ".csv";
    }
    std::ofstream file;
    file.open(FileName);
